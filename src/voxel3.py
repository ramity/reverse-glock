...
def carve_and_save_stl(image_paths, azimuths, tilts, K, distance, res=600, scale=200):
    """
    res: Voxel grid resolution (e.g., 100x100x100)
    scale: The size of the bounding box in world units
    """
    # 1. Initialize Voxel Grid (True = Solid)
    voxels = np.ones((res, res, res), dtype=bool)
    
    # Create coordinate grid
    grid_range = np.linspace(-scale/2, scale/2, res)
    X, Y, Z = np.meshgrid(grid_range, grid_range, grid_range, indexing='ij')
    # Flatten for matrix math: [4, N]
    world_pts = np.vstack([X.ravel(), Y.ravel(), Z.ravel(), np.ones(X.size)])
    ...
    for tilt in tilts:
        for azim in azimuths:
            ...
            voxels &= current_occupancy.reshape(res, res, res)

            idx += 1

    print("Extracting surface via Marching Cubes...")
    # 2. Convert Voxel Grid to Mesh
    # Marching cubes finds the boundary between True and False
    verts, faces, normals, values = measure.marching_cubes(voxels.astype(float), level=0)

    # Rescale vertices back to world coordinates
